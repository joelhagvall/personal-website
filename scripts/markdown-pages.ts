/**
 * Builds the markdown twin of every page for AI agents.
 *
 * Pure: returns { relativePath: markdown }. scripts/generate-markdown.ts
 * writes the result to public/ and out/.
 */
...
${PRIVACY_CONTENT.sections
  ...
  pages["404.md"] = pageMd(
    `Page not found - ${PERSON.name}`,
    `The requested URL does not exist on ${SITE.url} (HTTP 404).`,
    `## Where to look next

- [Home](${SITE.url}/) - start here
- [All pages](${SITE.url}/llms.txt) - machine-readable page index
- [Sitemap](${SITE.url}/sitemap.xml) - full URL list
- [Agent instructions](${SITE.url}/agents.md) - how to use this site`
  );
...
- Missing pages return a real HTTP 404; with \`Accept: text/markdown\` (or any non-HTML Accept header) the 404 carries a short markdown body with recovery links.
