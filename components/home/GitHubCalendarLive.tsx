...
// Public read-only API for GitHub contribution data (same source as
// react-github-calendar) — lets the graph refresh on page load instead of
// being frozen at build time. The build-time calendar is kept as fallback.
...
export function GitHubCalendarLive({
  ...
  useEffect(() => {
    ...
    fetch(`${CONTRIBUTIONS_API}/${username}?y=last`)
      ...
      .then(
        (data: {
          ...
          if (!cancelled && data?.contributions?.length) {
            setCalendar(
              contributionsToCalendar(data.contributions, data.total?.lastYear)
            );
          }
        }
      )
      .catch(() => {
        // Keep the build-time fallback on network/API failure
      });
  ...
  return <GitHubCalendar calendar={calendar} />;
}
